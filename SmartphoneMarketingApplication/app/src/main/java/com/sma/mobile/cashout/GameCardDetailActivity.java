...
public class GameCardDetailActivity extends AbstractAppCompatActivity implements View.OnClickListener  {
    ...
    GameCardResponse gameCardResponse;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        gameCardResponse = (GameCardResponse) getIntent().getExtras().getSerializable(GameCardResponse.class.getName());
        gameCardResponse.setQuantityOrdered(counterValue);
        getSupportActionBar().setTitle(gameCardResponse.getName());
        app_compat_text_view_title.setText(String.format("%s", gameCardResponse.getName()));
        app_compat_text_view_total_cost.setText(
                String.format("Đơn giá: %s", formatter.format(gameCardResponse.getPrice().multiply(new BigDecimal(counterValue))) + " đồng"
                ));
        app_compat_text_view_cost_converter.setText(
                String.format("Quy đổi: %s điểm", formatter.format(gameCardResponse.getPointExchange().multiply(new BigDecimal(counterValue))))
        );
...
//                        formatter.format(gameCardResponse.getPrice().multiply(new BigDecimal(counterValue))) + " đ"
//                ));
                app_compat_text_view_cost_converter.setText(
                        String.format("Quy đổi: %s điểm", formatter.format(gameCardResponse.getPointExchange().multiply(new BigDecimal(counterValue))))
                );
...
//        if (gameCardResponse.getQuantity() <= 0) {
...
//        }
    ...
    protected void onResume() {
        ...
        FintechvietSdk.getInstance().deleteCart(registrationToken, new JCallback<AbstractMessage>() {
            @Override
    ...
    public void onClick(View v) {
        FintechvietSdk.getInstance().addToCart(getDeviceSerialNumber(), gameCardResponse.getVoucherId(),
                gameCardResponse.getQuantityOrdered(), gameCardResponse.getPrice().toPlainString(),
                        /*phoneCardResponse.getType()*/"GAME_CARD", new JCallback<AbstractMessage>() {
                    @Override
                    public void onResponse(Call<AbstractMessage> call, Response<AbstractMessage> response) {
                        if (response.code() == 200) {
                            Intent intent = new Intent();
                            intent.setClassName(GameCardDetailActivity.this, OrderActivity.class.getName());
                            ...
                            try {
                                if (response.errorBody() != null) {
                                    ...
                                    getMaterialDialogAlert(GameCardDetailActivity.this, "Thông báo",
                    ...
                    @Override
                    public void onFailure(Call<AbstractMessage> call, Throwable t) {
                        getMaterialDialogAlert(GameCardDetailActivity.this, "Thông báo", "Xảy ra một lỗi từ máy chủ").show();
                    }
                });
    }
}
