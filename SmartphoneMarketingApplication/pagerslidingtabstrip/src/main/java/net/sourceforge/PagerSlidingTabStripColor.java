 ...
 */
public class PagerSlidingTabStripColor extends HorizontalScrollView {
    ...
    public PagerSlidingTabStripColor(Context context) {
        this(context, null);
    }

    public PagerSlidingTabStripColor(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @SuppressWarnings("ResourceType")
    public PagerSlidingTabStripColor(Context context, AttributeSet attrs, int defStyle) {
        ...
        // <<
        mTabLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT, 1.0f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mDividerWidth > 0) {
            ...
            for (int i = 0; i < mTabCount/* - 1*/; i++) {
                ...
                canvas.drawRect(tab.getLeft() + mPaddingLeft, 0, tab.getRight() + mPaddingLeft, height, mRectPaint);
            }
        }
        ...
        mRectPaint.setColor(0xFF6ca4fe/*Color.RED*//*mIndicatorColor*/);
        Pair<Float, Float> lines = getIndicatorCoordinates();
        //canvas.drawRect(lines.first + mPaddingLeft, 0/*height - mIndicatorHeight*/, lines.second + mPaddingLeft, height, mRectPaint);
        //canvas.drawRoundRect(new RectF(lines.first + mPaddingLeft, 0/*height - mIndicatorHeight*/, lines.second + mPaddingLeft, height), 20, 20, mRectPaint);
        //}
        if (isRoundRect()) {
            Path path = new Path();
            if (lines.first <= 0) {
                path.addRoundRect(
                        new RectF(lines.first + mPaddingLeft,
                                0/*height - mIndicatorHeight*/,
                                lines.second + mPaddingLeft,
                                height), 10, 10, Path.Direction.CCW);
                canvas.drawPath(path, mRectPaint);
                canvas.drawRoundRect(
                        new RectF(
                                lines.second + mPaddingLeft - 5,
                                0/*height - mIndicatorHeight*/,
                                lines.second + mPaddingLeft,
                                height), 0, 0, mRectPaint);
            } else {
                path.addRoundRect(
                        new RectF(lines.first + mPaddingLeft,
                                0/*height - mIndicatorHeight*/,
                ...
                canvas.drawRoundRect(
                        new RectF(
                                lines.first + mPaddingLeft - 1,
                                0/*height - mIndicatorHeight*/,
                                lines.first + mPaddingLeft + 5,
                                height), 0, 0, mRectPaint);
            }
        } else {
    ...
    private void unSelect(View tab) {
        if (tab != null) {
            ...
            if (tab_title != null) {
                ...
                //tab_title.setTextColor(Color.BLACK/*mTabTextColor*/);
            }
    ...
    private void select(View tab) {
        if (tab != null) {
            ...
            TextView tab_title = (TextView) tab.findViewById(R.id.tab_title_id);
            if (tab_title != null) {
                //tab_title.setTextColor(Color.WHITE/*mTabTextColor*/);
